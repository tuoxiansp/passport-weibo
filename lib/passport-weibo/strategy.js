...
function Strategy(options, verify) {
  ...
  this._getEmailAPI = options.getEmailAPI || 'https://api.weibo.com/2/account/profile/email.json';
    ...
    this._oauth2.getProtectedResource(this._getuidAPI, accessToken, function(err, body, res) {
        ...
        try {
            ...
            self._oauth2.getProtectedResource(self._getProfileAPI + '?uid=' + raw.uid, accessToken, function(err, body, res) {
                ...
                try {
                  var profile =  self.formatProfile(JSON.parse(body)); // this is different with raw.

                  //TODO add default email scope
                  try {
                    self._oauth2.getProtectedResource(self._getEmailAPI, accessToken, function(err, body, res) {
                      if (err) return callback(new InternalOAuthError('', err));
                      
                      var emailRes = JSON.parse(body);

                      profile.email = emailRes && emailRes[0] && emailRes[0].email;
